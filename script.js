document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Loaded. Setting up baseline features...");
    ...
    const visitCountSpan = document.getElementById('visit-count'); // For fake counter
    ...
    const locationString = "So,Bitter"; const typeSpeed = 180; const deleteSpeed = 120; const pauseDuration = 2500; let locationCharIndex = 0; let locationIsDeleting = false; let locationLoopTimeout;
    ...
    function tiltPopup(e, popupElement) { const centerX = window.innerWidth / 2; const centerY = window.innerHeight / 2; const deltaX = e.clientX - centerX; const deltaY = e.clientY - centerY; const maxRotate = 15; /* Tilt intensity */ const rotateY = -(deltaX / centerX) * maxRotate; const rotateX = (deltaY / centerY) * maxRotate; const clampedRotateX = Math.max(-maxRotate, Math.min(maxRotate, rotateX)); const clampedRotateY = Math.max(-maxRotate, Math.min(maxRotate, rotateY)); const tiltAmount = Math.min(1, Math.sqrt(deltaX**2 + deltaY**2) / (Math.sqrt(centerX**2 + centerY**2) * 0.7)); const translateZ = tiltAmount * 25; popupElement.style.transform = `translateX(-50%) translateZ(${translateZ}px) rotateX(${clampedRotateX}deg) rotateY(${clampedRotateY}deg)`; }
    function resetPopupTilt(popupElement) { if(popupElement) { popupElement.style.transform = `translateX(-50%) translateZ(0px) rotateX(0deg) rotateY(0deg)`; } }
